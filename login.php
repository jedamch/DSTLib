...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($loginInput === '' || $password === '') {
        ...
        /* Making sure that the password typed matches with the one in the database. */
        if ($user && password_verify($password, $user['Hashed_Password'])) {
            if ($user['Is_Banned']) {
                $errors[] = "Your account has been banned. Reason: " . htmlspecialchars($user['Ban_Reason'] ?? 'No reason given.'); /*Making sure that banned users cannot login.  */
            } else {
                $_SESSION['ID']        = $user['ID'];
