...
$success = "";
$userId  = $_SESSION['ID'];
/* Handling form info for the list. */
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        /* Making sure that the user cannot exceed 100 lists. */
        if ($count >= 100) {
            $errors[] = "You have reached the maximum of 100 lists.";
        } else {
            $slug = makeSlug($name); /* Creates unique slug. */
            ...
            exit; /* After list is created, it redirects to the list. */
        }
    }
}
?>
