...
if (!$author) die("Author not found."); /* Error message in case there is an invalid link. */

if ($_SERVER["REQUEST_METHOD"] === "POST") {

    
    /* Handles the editing of the author's photograph. */
    if (!empty($_POST['photograph'])) {
        $data = explode(",", $_POST['photograph'])[1];
        $data = base64_decode($data); /* Deletes old photo */

        $newphotograph = "photo_" . $authorId . "_" . time() . ".png";
        $uploadDir = __DIR__ . "/Authors/Photographs/"; /* Path to storage */

        if (!is_dir($uploadDir)) mkdir($uploadDir, 0755, true);

        file_put_contents($uploadDir . $newphotograph, $data); /* Naming the file. */
    ...
    /* Handling the editing of the name*/
    ...
    /* Handling the editing of the year of birth*/
    ...
    /* Handling the editing or addition of the year of death*/
    if ($newyod !== (string)$author['Year_Of_Death']) {
     ...
     /* Handling the editing or addition of the bio.*/
    if ($newbio !== $author['Bio']) {
     ...
     /* Handling the editing or addition of the place of birth */
