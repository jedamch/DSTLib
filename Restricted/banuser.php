...
/* Making sure that only valid users can be banned. */

if (empty($_GET['ID'])) die("No user ID provided.");
$userId = (int)$_GET['ID'];

/* Making sure that admins cannot ban themselves. */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    /* Redirects the admin to the user list page. */
    header("Location: users.php");
    exit;
}

/* Makes sure that the right user is picked.  */
