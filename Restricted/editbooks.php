...
/* Making sure that only admins use this. */
requirerole(['Admin']);
$success = "";

/* Message just in case an improper book id is used.  */
...
/* Handles editing of the book cover.  */
if (!empty($_POST['bookcover'])) {
    ...
    $stmt->execute(); /* Stores it in the aforementioned space.  */
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && empty($_POST['bookcover'])) {
    ...
    /* Handles the editing of the title*/
    if ($newtitle !== $book['Title']) {
     ...
     /* Handles the editing of the synopsis */
    if ($newsynopsis !== $book['Synopsis']) {
     ...
     /* Handles the editing of the year of release */
