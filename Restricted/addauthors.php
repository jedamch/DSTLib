...
/* Making sure that only admins can access this page.  */
requirerole(['Admin']);

/* Message text if and when an error is made.  */
$success = "";
$error = "";
/* PHP for when the admin enters the neccessary data to add an author. */
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    /* Requiring the user to enter the name and date of birth.  It allows the other fields to stay empty in case that information is unknown.*/
    if (empty($name) || empty($year_of_birth)) {
        ...
        /* Code to make sure that the photograph can be cropped and uploaded to the site, limiting the size of the photograph and deleting the old one after the new is done. */
        if (!empty($_POST['photograph'])) {
            ...
            $stmt->bind_param("si", $newauthorphotograph, $newAuthorId); 
            $stmt->execute();
        }
