...
/* Making sure the particular user exists.  If the user does not exist it will show a message saying 'User not found'. */
...
$profilePic    = $user['Avatar'] ?? 'default.png'; /* Default PNG is no image at all. */
$profilePicUrl = "Uploads/$profilePic"; /* File path for the images */
$isOwnProfile  = isset($_SESSION['User_Name']) && $_SESSION['User_Name'] === $user['User_Name']; /* To allow later editing of user profiles. */

/* This loads the most recent user comments on the profile which are submitted by other users. */
$stmt = $conn->prepare("
...
/* This loads the recent reviews created by the user, limiting it to 10. */
$stmt = $conn->prepare("
...
/* Loading the recent lists created by the user */
$listQuery = "
...
/* Uses the friend table to load the total number of friends the user has. */
$stmt = $conn->prepare("
...
/* For users logged in, this will allow them to see if the particular user is friends with them, */
$friendStatus = null;
$friendRow    = null;
...
/* Allows them to see pending friend requests on the profile.*/
...
<div class="profile-lists">
    ...
    <?php if ($userLists->num_rows > 0): ?>
        ...
        <?php endwhile; ?>
    <?php else: ?>
        <p>No lists yet.</p> <!-- Message for the reader if there are no lists. -->
    <?php endif; ?>
</div>
